...
from .resnet_50_block import build_ResNet_50_block
...
class ResNet(tf.keras.Model):
    def __init__(self, repeat_num_list):
        ...
        self.pool1 = tf.keras.layers.MaxPool2D(pool_size=(3, 3),
                                               strides=2,
                                               padding="same")
        self.block1 = build_ResNet_50_block(filters=64,
                                          strides=1,
                                          repeat_num=repeat_num_list[0])
        self.block2 = build_ResNet_50_block(filters=128,
                                          strides=2,
                                          repeat_num=repeat_num_list[1])
        self.block3 = build_ResNet_50_block(filters=256,
                                          strides=2,
                                          repeat_num=repeat_num_list[2])
        self.block4 = build_ResNet_50_block(filters=512,
                                          strides=2,
                                          repeat_num=repeat_num_list[3])
        self.pool2 = tf.keras.layers.GlobalAveragePooling2D()
        self.fc = tf.keras.layers.Dense(units=NUM_CLASSES,
...
def ResNet_50():
    return ResNet(repeat_num_list=[3, 4, 6, 3])